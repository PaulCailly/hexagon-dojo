...
export const BOOK: BookChapter[] = [
  {
    ...
    blocks: [
      ...
      { t: "p", c: "A boundary is a line in the code where knowledge stops flowing. On one side, the business logic knows nothing about vendors, protocols or configuration. On the other side, infrastructure code knows everything about one vendor and nothing about business rules. Ports, adapters and dependency injection are simply the three tools that make this line real instead of aspirational." },
      { t: "p", c: "One mental model to carry through the whole book: the business logic is the center of a hexagon, and every interaction with the outside world crosses one of its edges through a well-defined contract. That is why the pattern is also called hexagonal architecture. The shape does not matter. The edges do." },
      { t: "take", c: "Business logic describes WHAT happens, infrastructure describes HOW. A boundary is where knowledge stops flowing between the two. Everything else in this book is machinery for enforcing that line." },
    ],
  },
  {
    title: "Coupling and the Direction of Dependencies",
    blocks: [
      { t: "p", c: "Coupling is the degree to which changing one piece of code forces changes in another. Some coupling is unavoidable: a use case must somehow reach a database. The design question is never 'coupled or not', it is: in which DIRECTION does the dependency point, and toward what kind of code?" },
      { t: "p", c: "Code differs in volatility. Business rules change when the business changes: rarely, deliberately. Infrastructure changes constantly: SDK versions, API contracts, vendor migrations, pricing. The golden rule of dependable design is that stable things must not depend on volatile things. When your reward rules import the DynamoDB SDK, the most stable code in your system inherits the volatility of the least stable." },
      { t: "p", c: "The naive dependency direction is: use case, then database driver. The arrow points outward, from stable to volatile. Dependency inversion flips it. The use case depends on an abstraction it owns (a port), and the infrastructure implements that abstraction. Now both sides point at the interface, and the arrow from concrete infrastructure points INWARD, toward the business." },
      { t: "code", c: "WITHOUT inversion:\n  use case ---> DynamoDB SDK        (stable depends on volatile)\n\nWITH inversion:\n  use case ---> RewardRepository <--- DynamoDB adapter\n               (port, owned by the business)" },
      { t: "p", c: "This is the Dependency Inversion Principle, the D in SOLID: high-level modules should not depend on low-level modules, both should depend on abstractions, and abstractions should not depend on details. Notice something subtle and interview-relevant: dependency inversion is a design principle about direction. Dependency injection, which arrives in chapter 6, is a technique for supplying implementations. They are related but not synonyms, and confusing them is a common tell." },
      { t: "take", c: "Point every arrow inward. Stable business code must never depend on volatile infrastructure. Inversion is the principle (who depends on whom), injection is the technique (how implementations arrive)." },
    ],
  },
  {
    title: "Ports: Interfaces Owned by the Domain",
    blocks: [
      { t: "p", c: "A port is an interface defined by the inner logic that expresses a need. Every word of that sentence carries weight. Defined BY the inner logic: the domain writes it, owns it, and shapes it around its own vocabulary. Expresses a NEED: 'I need to fetch a reward', 'I need to trigger a redemption', 'I need to know the current time'. A port never describes a mechanism." },
      { t: "code", c: "interface RewardRepository {\n  getById(id: string): Promise<Reward | null>;\n}\n\ninterface RedemptionService {\n  redeem(userEmail: string, reward: Reward): Promise<void>;\n}\n\ninterface Clock {\n  now(): Date;\n}" },
      ...
      { t: "p", c: "Even time and randomness can be ports. A Clock port looks like over-engineering until you need to test 'offers expire at midnight' and discover your test only passes before noon. Wrapping Date.now behind a port makes time injectable, and therefore controllable. The same goes for ID generation and randomness. The rule of thumb: anything non-deterministic or external is a candidate for a port." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: "p", c: "The load-bearing rule is about imports: domain imports nothing from adapters, wiring, or any SDK. Adapters import domain types and ports, plus their own vendor. Wiring imports both, and is the only place that does. This is checkable by machine: an ESLint no-restricted-imports rule, or dependency-cruiser, fails the build when someone imports aws-sdk inside domain. 'The import graph is the architecture, and we lint it' is a strong sentence in a system design discussion." },
      { t: "p", c: "Naming carries information too. The codebase's Creator suffix (dynamoDbRewardRepositoryCreator) signals a factory that receives dependencies and returns a port implementation. Ports are named for needs (RewardRepository, Clock), adapters for technology plus port (dynamoDbRewardRepository), use cases for business actions (redeemReward). A reader should be able to classify any file from its name alone, which is exactly the skill the classify drill in this app trains." },
      ...
      { t: "take", c: "Make the boundary visible in folders and enforceable in CI. Domain imports nothing external, only wiring imports everything, and a lint rule turns the architecture from a convention into a guarantee." },
    ],
  },
  {
    title: "A Catalogue of Smells",
    blocks: [
      { t: "p", c: "Code review is pattern matching. This catalogue lists the violations you are most likely to be shown in the interview, each with the reason it hurts and the sentence to say when you spot it." },
      { t: "li", c: [
      ...
      { t: "p", c: "Notice the shared shape of every response: name the smell, state the principle in one clause, propose the move. Diagnosis, principle, remedy. In a live review, that rhythm reads as seniority. Ranting about bad code without the remedy, or fixing without naming why, both read as less." },
      { t: "take", c: "Every smell is knowledge in the wrong place: vendor knowledge in the domain, rule knowledge in an adapter, wiring knowledge in a workflow. Review by asking of each line: does this code KNOW something it should not?" },
    ],
  },
  {
    title: "Talking the Talk: Glossary and Interview Phrasing",
    blocks: [
      { t: "p", c: "The interview is in English, and precision of vocabulary is part of the signal. This closing chapter is the language layer: the terms, and the sentences that deploy them." },
      { t: "li", c: [
      ...
      { t: "p", c: "Sentence templates to make automatic, so your working memory stays free for the actual problem. For reasoning: 'Let me walk you through my reasoning.' 'My first instinct is X, but let me check the boundary.' 'The trade-off I see is speed of delivery against cost of change.' For review: 'What this adapter knows here is a business rule, so I would move it up into the use case.' For design: 'I would keep the domain vendor-neutral and absorb that concern in a decorator at the edge.' For the AI assistant: 'I will let the assistant scaffold the adapter, and I will review it for leaked rules, because boundary decisions stay with me.'" },
      { t: "p", c: "And one closing frame for the product engineering phase: a product built on merchant APIs, banking connections and reward partners lives or dies by how cheaply it can change partners. Ports make each partner a plug. That is not clean code for its own sake, it is the architecture of business agility, and connecting those two ideas in one sentence is the strongest note you can end an interview on." },
      { t: "take", c: "Precision of language is precision of thought made audible. Master the twelve takeaway boxes, the glossary, and the sentence templates, and the English of this interview becomes a home game." },
    ],
  },
];
