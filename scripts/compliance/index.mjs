...
function listFiles() {
  ...
  for (const root of config.roots) {
    ...
    for (const entry of fs.readdirSync(scanRoot, { recursive: true })) {
      ...
      if (
        config.include.test(rel) &&
        !config.exclude.test(rel) &&
...
try {
  const vercelPath = baseDir
    ? path.join(baseDir, "vercel.json")
    : "vercel.json";
  const rewrites =
    JSON.parse(fs.readFileSync(vercelPath, "utf8")).rewrites ?? [];
...
const penalty = findings.reduce(
  (s, f) => s + (config.points[f.severity] ?? 0),
  0,
);
...
const kpis = {
  ...
  controlCoveragePct: automated.length
