...
export const config = {
   ...
   */
  exclude:
    // src/content holds teaching material: code samples quoted as string data
    // (fake api.bank.com / ifeelgoods URLs). They are never executed, so they
    // are not egress.
    /(\.test\.|\.spec\.|\.stories\.|\.d\.ts$|node_modules\/|\/dist\/|\/\.next\/|\/build\/|\/coverage\/|\/__tests__\/|\/tests\/|\/test\/|src\/content\/)/,
  ...
  secretPatterns: [
    ...
    {
      id: "jwt-literal",
      re: /\beyJ[A-Za-z0-9_-]{12,}\.[A-Za-z0-9_-]{12,}\.[A-Za-z0-9_-]{8,}/,
    },
  ...
  analytics: {
    ...
    vendorCapture:
      /\bposthog\s*\.\s*(?:capture|identify|register|setPersonProperties)\s*\(/,
